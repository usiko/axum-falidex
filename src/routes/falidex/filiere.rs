...
use super::model::{FiliereCreateReq, FiliereUpdateReq};
use axum::{
    Json,
    extract::{Path, State},
...
pub async fn create(
    ...
    Json(payload): Json<FiliereCreateReq>,
) -> Response {
    let user_id = token.sub;
    let filiere = payload.into();
    match filiere::create(&state.db.db, user_id, filiere).await {
        Ok(message) => (
            StatusCode::CREATED,
            Json(json!({
                "success": true,
...
pub async fn update(
    ...
    State(state): State<AppState>,
    Path(id): Path<String>,
    Json(payload): Json<FiliereUpdateReq>,
) -> Response {
    let user_id = token.sub;
    let filiere = payload.into();
    match filiere::update(&state.db.db, user_id, id, filiere).await {
        Ok(message) => (
            StatusCode::OK,
            Json(json!({
                "success": true,
...
pub async fn delete(
    ...
    let user_id = token.sub;
    match filiere::delete(&state.db.db, user_id, id).await {
        Ok(message) => (
            StatusCode::OK,
            Json(json!({
                "success": true,
...
pub async fn get_occurences(State(state): State<AppState>, Path(id): Path<String>) -> Response {
    match filiere::get_occurences(&state.db.db, id).await {
