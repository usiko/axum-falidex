...
pub async fn create(
    ...
    match symbole_sens::create(&state.db.db, user_id, symbole_sens).await {
        Ok(message) => (
            StatusCode::CREATED,
            Json(json!({
                "success": true,
...
pub async fn update(
    ...
    let user_id = token.sub;
    let symbole_sens = payload.into();
    match symbole_sens::update(&state.db.db, user_id, id, symbole_sens).await {
        Ok(message) => (
            StatusCode::OK,
            Json(json!({
                "success": true,
...
pub async fn delete(
    ...
    let user_id = token.sub;
    match symbole_sens::delete(&state.db.db, user_id, id).await {
        Ok(message) => (
            StatusCode::OK,
            Json(json!({
                "success": true,
...
pub async fn get_occurences(State(state): State<AppState>, Path(id): Path<String>) -> Response {
    match symbole_sens::get_occurences(&state.db.db, id).await {
