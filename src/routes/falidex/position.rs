...
pub async fn get(State(state): State<AppState>) -> Response {
    match position::get(&state.db.db).await {
...
pub async fn create(State(state): State<AppState>, Json(payload): Json<PositionReq>) -> Response {
    let position = payload.into();
    match position::create(&state.db.db, position).await {
        Ok(message) => (
            StatusCode::CREATED,
            Json(json!({
                "success": true,
...
pub async fn update(
    ...
    let position = payload.into();
    match position::update(&state.db.db, id, position).await {
        Ok(message) => (
            StatusCode::OK,
            Json(json!({
                "success": true,
...
pub async fn delete(State(state): State<AppState>, Path(id): Path<String>) -> Response {
    match position::delete(&state.db.db, id).await {
        Ok(message) => (
            StatusCode::OK,
            Json(json!({
                "success": true,
